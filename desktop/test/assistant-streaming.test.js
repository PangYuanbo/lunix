const assert = require('assert');
const fs = require('fs');

const app = fs.readFileSync(require.resolve('../app.js'), 'utf8');
const html = fs.readFileSync(require.resolve('../index.html'), 'utf8');

assert.match(html, /assistant\/assistant\.css/);
assert.match(html, /assistant\/assistant\.js/);
assert.match(app, /LunixAssistant\?\.mount\(log\)/);
assert.match(app, /sessions\.chatSocket\(sessionId\)/);
assert.match(app, /frame\.type === 'delta'/);
assert.match(app, /aborted \|\| frame\.preview/);
assert.match(app, /frame\.type === 'done'/);
assert.match(app, /frame\.type === 'turn_done'/);
const doneBranch = app.match(/if \(frame\.type === 'done'\) \{([\s\S]*?)\n      \}/)?.[1] || '';
assert.doesNotMatch(doneBranch, /finishTurn\(\)/);
assert.match(app, /JSON\.stringify\(\{ type: 'send', text:/);
assert.match(app, /pushEvents\(restored\.events, true\)/);
assert.match(app, /data-new-chat/);
assert.match(app, /sessions\.start\(agentSession\.agentId\)/);
assert.match(app, /showLoading\('Starting a new chat'/);
assert.match(app, /showLoading\('Loading your conversation'/);
assert.match(app, /input\.disabled = locked/);
assert.match(app, /lockComposer\(true, true\)/);
assert.match(app, /Connection timed out/);
assert.match(app, /Lunix cannot confirm the task state/);
assert.match(app, /Agent connection is not ready/);
assert.doesNotMatch(app, /sessions\.sendMessage\(agentSession\.session\.id/);
assert.match(app, /showLoading\('Starting Codex'/);
assert.match(app, /showLoading\('Starting Claude'/);
assert.match(app, /Connection interrupted/);
assert.match(app, /Agent is still working/);
assert.match(app, /workingDetailOffset = Math\.floor\(Math\.random\(\) \* workingDetails\.length\)/);
assert.match(app, /Verifying that the result matches your request/);
assert.match(app, /progressSilent >= 30000/);
assert.match(app, /connectionSilent >= 10000/);
assert.doesNotMatch(app, /No recent output/);
assert.doesNotMatch(app, /Agent may be stalled/);
assert.doesNotMatch(app, /progressSilent >= 90000/);
assert.match(app, /lastProgressAt = Date\.now\(\)/);
assert.match(app, /nodusClient\.sessions\.pause\(agentSession\.session\.id\)/);
assert.match(app, /statusEl\.textContent = 'stop failed'/);
assert.match(app, /title: 'Could not stop the agent'/);
assert.match(app, /reconnectAttempts < 1/);
assert.match(app, /else if \(sending\) finishTurn\(\)/);
assert.match(app, /else \{ clearTaskState\(\); hideLoading\(\); \} return;/);
assert.match(app, /if \(frame\.activity\) mountAssistant\(\)\.setTask/);
assert.match(app, /sendBtn\.onclick = \(\) => \{ if \(sending\) stopTask\(\)/);
assert.doesNotMatch(app, /function bubble\(/);
assert.doesNotMatch(app, /function paintLog\(/);
console.log('Assistant Streamdown and WebSocket wiring ok');
