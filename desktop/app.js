...
function openVscodeLauncher() {
  ...
  overlay.querySelector('[data-desktop]').onclick = async (event) => {
    ...
    try {
      ...
      const alias = `lunix-${sessionId.slice(0, 8)}`, keyName = `${alias}-key`, downloadName = `${keyName}-${Date.now()}`;
      if (!/^[\w.-]+$/.test(connection.host) || !/^[\w.-]+$/.test(connection.user) || !Number.isInteger(connection.port) || connection.port < 1 || connection.port > 65535 || !/^\/[\w./-]*$/.test(connection.workspacePath || '')) throw new Error('Invalid SSH connection details.');
      const config = `Host ${alias}\n  HostName ${connection.host}\n  Port ${connection.port}\n  User ${connection.user}\n  IdentityFile ~/.ssh/${keyName}\n  IdentitiesOnly yes\n  StrictHostKeyChecking accept-new\n`;
      const marker = `lunix:${alias}`, config64 = btoa(config), vscodeUrl = `vscode://vscode-remote/ssh-remote+${encodeURIComponent(alias)}${connection.workspacePath}`;
      const setup = `mkdir -p "$HOME/.ssh" && mv "$HOME/Downloads/${downloadName}" "$HOME/.ssh/${keyName}" && chmod 600 "$HOME/.ssh/${keyName}" && touch "$HOME/.ssh/config" && tmp=$(mktemp) && awk '$0=="# ${marker} begin"{skip=1;next}$0=="# ${marker} end"{skip=0;next}!skip{print}' "$HOME/.ssh/config" > "$tmp" && printf '\\n# ${marker} begin\\n' >> "$tmp" && printf '%s' '${config64}' | /usr/bin/base64 -D >> "$tmp" && printf '# ${marker} end\\n' >> "$tmp" && mv "$tmp" "$HOME/.ssh/config" && chmod 600 "$HOME/.ssh/config" && open '${vscodeUrl}'`;
      overlay.firstElementChild.innerHTML = `<div style="padding:3px 4px 6px;"><strong style="display:block;font-size:13px;">VS Code Desktop</strong><span style="font-size:11px;color:#777;">Two steps on this Mac.</span></div><button data-key>1. Download private key</button><button data-setup disabled>2. Copy setup command</button><small data-help style="color:#777;line-height:1.45;padding:2px 4px;">Download the key first. Then paste the setup command into Terminal; it installs the SSH host and opens VS Code automatically.</small>`;
      ...
      const setupButton = overlay.querySelector('[data-setup]'), help = overlay.querySelector('[data-help]');
      overlay.querySelector('[data-key]').onclick = (keyEvent) => { const url = URL.createObjectURL(new Blob([connection.privateKey], { type: 'application/octet-stream' })); const link = document.createElement('a'); link.href = url; link.download = downloadName; link.click(); URL.revokeObjectURL(url); keyEvent.currentTarget.textContent = '1. Key downloaded'; setupButton.disabled = false; };
      setupButton.onclick = async () => { await navigator.clipboard.writeText(setup); setupButton.textContent = '2. Command copied'; help.textContent = 'Open Terminal, paste the command, and press Return. VS Code will open the workspace automatically.'; };
...
function renderAgent(root) {
  ...
  function connectChat() {
    ...
    chatSocket.onmessage = async (event) => {
      ...
      if (frame.type === 'agent_event' && frame.event) { lastProgressAt = Date.now(); mountAssistant().agentEvent(frame.event); return; }
      if (frame.type === 'delta') {
